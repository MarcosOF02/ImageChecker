...
class checkRangeDia(QThread):
    valoresNormales = pyqtSignal(str,str,str,str,str,str,str,str,str)
    valoresQuemada = pyqtSignal(str,str,str,str,str,str,str,str,QPixmap)
    valoresBlurred = pyqtSignal(str)
    ...
    def Roi(self,xmin,ymin,xmax,ymax,brillo):
        ...
        try:
            ...
            print("ERROR::DIA::DIRECTORIO DE IMAGENES NO EXISTE")
            self.work = False
        self.start()
    ...
    def valoresBase(self):
        ...
        for i in range(len(self.imageList)):
            ...
            print(f"INFO::BASE::DIA::PROCESANDO: {self.imageList[i]}")
        ...
        self.valoresNormales.emit(media_s,moda_s,medmod_s,media_v,moda_v,medmod_v,max_v,max_s,fm)
    ...
    def valoresBrilloMod(self):
        ...
        for i in range(len(self.imageList)):
            print(f"INFO::BRILLOMOD::DIA::PROCESANDO: {self.imageList[i]}")
            ...
            if not (os.path.isdir("../IMG-OUT/dia/brillo/")):
                os.makedirs("../IMG-OUT/dia/brillo/")

            cv2.imwrite("../IMG-OUT/dia/brillo/" + self.imageList[i], cv2.putText(cv2.cvtColor(imOriginal, cv2.COLOR_BGR2RGB),f"S(media): {media_s[-1]} V(media): {media_v[-1]} Brillo: {self.brillo}",(imOriginal.shape[1]- 400,(imOriginal.shape[0]-30)),color=(0,0,0),fontFace=1,fontScale=1.2,thickness=2)) 
        ...
        self.valoresQuemada.emit(media_s,moda_s,medmod_s,media_v,moda_v,medmod_v,max_v,max_s,hsvimage)
    ...
    def valoresBlurMod(self):
        ...
        for i in range(len(self.imageList)):
            ...
            if not (os.path.isdir("../IMG-OUT/dia/blur/")):
                os.makedirs("../IMG-OUT/dia/blur/")

            cv2.imwrite("../IMG-OUT/dia/blur/" + self.imageList[i], cv2.putText(cv2.cvtColor(imOriginal, cv2.COLOR_BGR2RGB),f"fm: {round(int(cv2.Laplacian(gray, cv2.CV_64F,ksize=5).var()))}",(imOriginal.shape[1]- 220,(imOriginal.shape[0]-30)),color=(0,0,0),fontFace=1,fontScale=1.2,thickness=2))
