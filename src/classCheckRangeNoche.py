...
class checkRangeNoche(QThread):
    valoresNormalesNoche = pyqtSignal(str,str,str,str,str,str,str,str,str)
    valoresQuemadaNoche = pyqtSignal(str,str,str,str,str,str,str,str,QPixmap)
    valoresBlurredNoche = pyqtSignal(str)
    ...
    def __init__(self, imagesDir,appDir):
        ...
        self.appDir = appDir
        self.imagesDir = imagesDir + "noche/"
        self.work = True
    ...
    def Roi(self,xmin,ymin,xmax,ymax,brillo):
        ...
        try:
            ...
            print("ERROR::NOCHE::DIRECTORIO DE IMAGENES NO EXISTE")
            self.work = False
        self.start()
    ...
    def valoresBase(self):
        ...
        for i in range(len(self.imageList)):
            ...
            print(f"INFO::BASE::NOCHE::PROCESANDO: {self.imageList[i]}")
        ...
        self.valoresNormalesNoche.emit(media_s,moda_s,medmod_s,media_v,moda_v,medmod_v,max_v,max_s,fm)
    ...
    def valoresBrilloMod(self):
        ...
        for i in range(len(self.imageList)):
            print(f"INFO::BRILLOMOD::NOCHE::PROCESANDO: {self.imageList[i]}")
            ...
            if not (os.path.isdir("../IMG-OUT/noche/brillo/")):
                os.makedirs("../IMG-OUT/noche/brillo/")

            cv2.imwrite("../IMG-OUT/noche/brillo/" + self.imageList[i], cv2.putText(cv2.cvtColor(imOriginal, cv2.COLOR_BGR2RGB),f"S(media): {media_s[-1]} V(media): {media_v[-1]} Brillo: {self.brillo}",(imOriginal.shape[1]- 400,(imOriginal.shape[0]-30)),color=(0,0,0),fontFace=1,fontScale=1.2,thickness=2))
        ...
        self.valoresQuemadaNoche.emit(media_s,moda_s,medmod_s,media_v,moda_v,medmod_v,max_v,max_s,hsvimage)
    ...
    def valoresBlurMod(self):
        ...
        for i in range(len(self.imageList)):
            ...
            if not (os.path.isdir("../IMG-OUT/noche/blur/")):
                os.makedirs("../IMG-OUT/noche/blur/")

            cv2.imwrite("../IMG-OUT/noche/blur/" + self.imageList[i], cv2.putText(cv2.cvtColor(imOriginal, cv2.COLOR_BGR2RGB),f"fm: {round(int(cv2.Laplacian(gray, cv2.CV_64F,ksize=5).var()))}",(imOriginal.shape[1]- 220,(imOriginal.shape[0]-30)),color=(0,0,0),fontFace=1,fontScale=1.2,thickness=2))       


        fm = str(round(np.mean(fm)))

        self.valoresBlurredNoche.emit(fm)
